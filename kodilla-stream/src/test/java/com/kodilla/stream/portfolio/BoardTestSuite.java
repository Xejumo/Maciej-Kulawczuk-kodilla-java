...
class BoardTestSuite {
    ...
    private Board prepareTestData() {
        ...
        Task task1 = new Task("Microservice for taking temperature",
                "Write and test the microservice taking\n" +
                        "the temperature from an external service",
                user1,
                user2,
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        double averageDays = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .filter(task -> task.getDeadline().isAfter(LocalDate.now()) || task.getDeadline().isEqual(LocalDate.now()))
                .mapToDouble(task -> {
                    LocalDate created = task.getCreated();
                    LocalDate now = LocalDate.now();
                    long daysBetween = ChronoUnit.DAYS.between(created, now);
                    return daysBetween + task.getDuration();
                })
                .average()
                .orElse(0);

        // Then
        assertEquals(5.0, averageDays, 0.01);
    }
}
